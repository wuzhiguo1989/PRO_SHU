...
volatile u8 restart_read_status_signal = 0;
...
count_down_t xdata resend_restart_count_down;
count_down_t xdata open_delay;
count_down_t xdata lock_delay;
...
void resend_restart_command_call_back()
{
...
void open_delay_callback(){
    lock_on_detect_signal = 0;
}

void lock_delay_callback(){
...
void count_down_at_all_task()
{
    ...
    count_down_at(&resend_restart_count_down);
    // count_down_at(&open_delay);
    // count_down_at(&lock_delay);
}
...
void count_down_init_all_task()
{
    ...
    count_down_init(&resend_restart_count_down,resend_restart_command_call_back);
    // count_down_init(&open_delay, open_delay_callback);
    // count_down_init(&lock_delay, lock_delay_callback);

}

void reload_resend_restart_count_down(){
    count_down_reload(&resend_restart_count_down, 1, 0);
}

void cancel_resend_restart_count_down(){
    count_down_cancel(&resend_restart_count_down);
}
...
{ 
   count_down_reload(&open_count_down, 2, 0);
   power_open_work();
}
...
void reload_open_delay(){
    // count_down_reload(&open_delay, 0, 20); 
}

void reload_lock_delay(){
    // count_down_reload(&lock_delay, 0, 20);
}


void cancel_open_delay(){
    count_down_cancel(&open_delay);
}

void cancel_lock_delay(){
    count_down_cancel(&lock_delay);
}
