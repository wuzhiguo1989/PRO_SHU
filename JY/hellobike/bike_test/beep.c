...
static code u16 const pca_next_param[6] = {T100Hz, T4kHz, T800Hz, T600Hz, T600Hz, T600Hz};
...
volatile bit status_in_sleep = 1;
...
void pca_isr() interrupt 7 using 1 
{
    ...
    if(!speak_mode){
        ...
        if(pca_param >= T2kHz)
            pca_param = T4kHz;
   } else if(speak_mode == trace_beep || speak_mode == open_beep || speak_mode == lock_beep){
        speak_beep_enable();
        pca_next_target =  pca_next_target + pca_next_param[speak_mode];
        CCAP0L = pca_next_target;
        CCAP0H = pca_next_target >> 8;
