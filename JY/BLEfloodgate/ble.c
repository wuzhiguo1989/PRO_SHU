...
static volatile u8 xdata RX1_Spare[COM_RX1_Lenth];
static volatile u8 xdata RX1_Spare_Cnt = 0;
...
void init_A()
{
    u8 counter_for_A[4] = {0x00, 0x01, 0x02, 0x03};
...
static void GetString1(u8 *received) {
    ...
    // COM1.RX_Cnt = RX1_Spare_Cnt;
    COM1.RX_Cnt = 0;
    COM1.B_RX_OK = FALSE;
    memset(RX1_Buffer, 0, COM_RX1_Lenth);
    // if (RX1_Spare_Cnt) 
    //     memcpy(RX1_Buffer, RX1_Spare, RX1_Spare_Cnt);
    // RX1_Spare_Cnt = 0;
}
...
void BLE_Init() {
    ...
    memset(RX1_Buffer, 0, COM_RX1_Lenth);
    memset(RX1_Spare, 0, COM_RX1_Lenth);
    USART_Init(&COM1);
...
bool Check_Requset_for_Open_floodgate(u8 *received)
{
    ...
    //counter_for_Open_flood_door = 0x00000001;
...
{
    u8 timestamp[6] , i, key_cabinet_number, list_for_Open_key_cabinet[4], counter_for_B[4], order_over_creat_for_B[4];
    ...
    for(i = 0; i < 6; i++)
        timestamp[i] = ~ received[i+10];
    // 第8字节柜门编号
    key_cabinet_number = ~ received[8];
    if(CheckComplement(list_for_Open_key_cabinet, 1, 3, received, 1, 3) && CheckExclusive_Or(order_over_creat_for_B, 0, 3, received, 0, 3,  received, 4, 7) && (received[9] == key_cabinet_number ^ received[3]) && ChecksimpleCRC(received, 0, 7, received[16]) && ChecksimpleCRC(received, 8, 15, received[17]) && CheckAnd(received, 0, 17, received[18], received[19]))
    {
        // Store_Timestamp(timestamp, 0, 5);
        Store_Counter_for_Open_key_cabinet(counter_for_Open_key_cabinet);
        return 0;
    }
    else
        return 1;
...
void Send_Response_for_Open_key_cabinet(u8 *received)
{
    ...
    { 
        ...
        for(i = 2; i <= 7; i++ )
            send_data[i] = timestamp[i-2];
        send_data[8] = CRC_Creat(send_data, 0, 7);
        send_data[9] = (~ send_data[1]) ^ (~ received[8]);
        send_data[10] = (u8) (~ (AND_Creat(send_data, 0, 7) >> 8));
        send_data[11] = (u8) (~ AND_Creat(send_data, 0, 7));
...
bool Check_Requset_for_ChangeBLEname(u8 *received)
{
    ...
    //counter_for_change_BLEname = 0xFFFFFFFF;
...
void Send_Response_for_ChangeBLEname(u8 *received)
{
    ...
    { 
        ...
        //BLE_ChangeBLEName("123456789002");
        Delay50ms();
        // Delay2000ms();		
...
bool Read_status_key_cabinet(u8 key_cabinet_number)
{
  return 1;
}
...
{
    u8 send_data[20], i, code_key_cabinet;
    memset(send_data, 0 , 20);
    send_data[0] = 0x15;
    ...
    if(RI)
    {
        if(COM1.B_RX_OK == 0)
        {
            ...
            if(received_for_ble_response)
            {
                ...
                // else if(strcmp(RX1_Buffer, BLE_IND_CONNECT_BREAK) == 0)
                // {
                //     COM1.B_RX_OK = 1;
                //     received_for_ble_response = 0;
                //     INT_PROC |= BLE_DUTY;
            ...
            // use spare buffer to store 
            if (RX1_Spare_Cnt < COM_RX1_Lenth)
                RX1_Spare[RX1_Spare_Cnt++] = SBUF;
        }
        RI = 0;
    }
...
//             {
//                 RX1_Buffer[COM1.RX_Cnt] = SBUF;
//                 if(SBUF == 'K' && COM1.RX_Cnt == 1 && RX1_Buffer[0] == 'O')
//                 {
//                     COM1.B_RX_OK = 1;
//                     received_for_ble_response = 0;
//                     INT_PROC |= BLE_DUTY;
