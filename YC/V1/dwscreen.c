...
void display_all_para(u16 addr)
{
	TX_write2buff(COM[COM2_ID], 0x5A);
	TX_write2buff(COM[COM2_ID], 0xA5);
	TX_write2buff(COM[COM2_ID], 0x1F);
	TX_write2buff(COM[COM2_ID], 0x82);
	TX_write2buff(COM[COM2_ID], addr >> 8);
...
void updata_data_from_dwscreen()
{
	...
	if(time_feed_suction > time_feed_intercept)
		time_feed_intercept_set = time_feed_suction - time_feed_intercept;
	else 
		time_feed_intercept_set = 0;
...
void dw_data_reading(u8 *response) {
		...
		time_feed_intercept = response[16];
    	time_feed_discharge = response[18];
    	time_feed_lack = response[20];
		counter_for_lack_alter = response[22];
    	time_fan_delay_stop = response[24];
		time_fan_delay_full = response[26];
		tem_alter = response[28];
		tem_stop = response[30];
		time_feed_stop = response[32];
