...
u32 password_super = 0;

#define AVAILBALE_TIME 6
...
void password_init()
{
    ...
    password_super = EEPROM_read(ADDRESS_FOR_PASSWORD_SUPER_INIT) << 24 | EEPROM_read(ADDRESS_FOR_PASSWORD_SUPER_INIT + 1) << 16 | EEPROM_read(ADDRESS_FOR_PASSWORD_SUPER_INIT + 2) << 8 | EEPROM_read(ADDRESS_FOR_PASSWORD_SUPER_INIT + 3);
}
...
void param_init()
{
    ...
    // 从INIT中取
        ...
        time_feed_intercept = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 2);
        time_spray_wash = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 3);
        time_spray_wash_between = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 4);
        counter_for_spray_wash = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 5);
        tem_alter = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 6);
        tem_stop = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 7);
        // time_feed_stop = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 8);
        time_spray_wash_before_stop = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 8);
        time_spray_wash_between_before_stop = EEPROM_read(ADDRESS_FOR_PARAM_INIT + 9); 
    ...
    // 从SET中取
        ...
        time_feed_intercept = EEPROM_read(ADDRESS_FOR_PARAM_SET + 3);
        time_spray_wash = EEPROM_read(ADDRESS_FOR_PARAM_SET + 4);
        time_spray_wash_between = EEPROM_read(ADDRESS_FOR_PARAM_SET + 5);
        counter_for_spray_wash = EEPROM_read(ADDRESS_FOR_PARAM_SET + 6); 
        // time_feed_lack = EEPROM_read(ADDRESS_FOR_PARAM_SET + 9);
        // counter_for_lack_alter = EEPROM_read(ADDRESS_FOR_PARAM_SET + 10);
        tem_alter = EEPROM_read(ADDRESS_FOR_PARAM_SET + 7);
        tem_stop = EEPROM_read(ADDRESS_FOR_PARAM_SET + 8);
        // time_feed_stop = EEPROM_read(ADDRESS_FOR_PARAM_SET + 9);
        // feed_lack_signal = EEPROM_read(ADDRESS_FOR_PARAM_SET + 14);
        time_spray_wash_before_stop = EEPROM_read(ADDRESS_FOR_PARAM_SET + 9);
        time_spray_wash_between_before_stop = EEPROM_read(ADDRESS_FOR_PARAM_SET + 10); 
        updata_data_from_dwscreen();
    }    
}
...
void store_all_para()
{
    ...
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 3, time_feed_intercept);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 4, time_spray_wash);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 5, time_spray_wash_between);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 6, counter_for_spray_wash);
    // EEPROM_write(ADDRESS_FOR_PARAM_SET + 9, time_feed_lack);
    // EEPROM_write(ADDRESS_FOR_PARAM_SET + 10, counter_for_lack_alter);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 7, tem_alter);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 8, tem_stop);
    // EEPROM_write(ADDRESS_FOR_PARAM_SET + 9, time_feed_stop);
    // EEPROM_write(ADDRESS_FOR_PARAM_SET + 14, feed_lack_signal);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 9, time_spray_wash_before_stop);
    EEPROM_write(ADDRESS_FOR_PARAM_SET + 10, time_spray_wash_between_before_stop);
}
...
{
    if(EEPROM_read(ADDRESS_FOR_VAILABLE_FLAG) == 0xFF){
        // 无需比较时间、同步图标
        syc_available_siganl(0);
        sigal_available = 0;
        display_init_deadline(16, 1, 1);
        page_back_home(); 
    } else {
        // 读时间
        syc_available_siganl(1); 
        time_year = EEPROM_read(ADDRESS_FOR_VAILABLE_SET + 1);
        time_month = EEPROM_read(ADDRESS_FOR_VAILABLE_SET + 2);
        time_day = EEPROM_read(ADDRESS_FOR_VAILABLE_SET + 3);
        display_init_deadline(time_year, time_month, time_day);
        sigal_available = 0;
        Delay();          
        get_RTC_from_screen(); 
    }
...
{
    EEPROM_SectorErase(ADDRESS_FOR_VAILABLE_FLAG);
    EEPROM_write(ADDRESS_FOR_VAILABLE_FLAG, 1);
    // 处理时间
            TX_write2buff(&COM1, time_start[8]);
        TX_write2buff(&COM1, time_start[10]);
        TX_write2buff(&COM1, time_start[12]);
    // 年
    EEPROM_write(ADDRESS_FOR_VAILABLE_SET + 1, time_start[8]);
    // 月
    EEPROM_write(ADDRESS_FOR_VAILABLE_SET + 2, time_start[10]);
    // 日 
    EEPROM_write(ADDRESS_FOR_VAILABLE_SET + 3, time_start[12]);
}
