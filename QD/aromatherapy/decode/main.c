...
void parse_crtl_signal(u8 receive_data)
{
              
  if(receive_data == 21 && !command_counter)
    {
        // 模式
        MODE_DUTY = 1;
        memset(syc_data_raw, 0, 9);
  ...
  else if(receive_data == 29 && !command_counter)
    {
        // 定时关机
        ALARM_DUTY = 1;
    memset(syc_data_raw, 0, 9);
    ...
    {   
      if(MODE_DUTY)
        ...
        if(POWER_DUTY)
        {
            ...
            if(command_counter == 2)
            {
                Bk8000l_Change_Power(syc_data_raw[1]);
                command_counter = 0;
                POWER_DUTY = 0;
            }
        }
        if(COLOR_DUTY)
        {
            ...
            {
                Bk8000l_Sync_Color(syc_data_raw[1] << 8 | syc_data_raw[2], syc_data_raw[4] << 8 | syc_data_raw[5], syc_data_raw[7] << 8 | syc_data_raw[8]);
                command_counter = 0;
            }
        }
      if(ALARM_DUTY)
