...
volatile led_t xdata led_config_set[12], led_config_logic[12];
...
void ctrl_lumins_changed(const u8 lumins) {
	...
	if (check_lumins_level(lumins))
	{
		for (i = 0; i < 12; ++i)
		{
			led_config_logic[12].lumins = lumins;
		}
...
static void fire_work_half_cycle_reached() {
	...
	if(cnt_for_one_cycle_fire_work == 6)
	{
		cnt_for_one_cycle_fire_work = 0;
	...
	if(!cnt_for_cycle_fire_work)
	{
		for(i = 0; i < 12; i++)
		{
			if(i == cnt_for_one_cycle_fire_work || i == 11 -cnt_for_one_cycle_fire_work)
...
static void hot_wheels_half_cycle_reached() {
	...
	if(cnt_for_one_cycle_hot_wheels == 12)
	{
		cnt_for_one_cycle_hot_wheels = 0;
...
static void rainbow_chasing_half_cycle_reached() {
	...
	if(cnt_for_cycle_rainbow_chasing == 12)
		cnt_for_cycle_rainbow_chasing = 0;
	if(!cnt_for_cycle_rainbow_chasing)
	{
		...
		led_config_logic[5].red = 255;
		led_config_logic[5].blue = 203;
	} else if(cnt_for_cycle_rainbow_chasing == 1 || cnt_for_cycle_rainbow_chasing == 11){
		// 红色
	...
	} else if(cnt_for_cycle_rainbow_chasing == 2  || cnt_for_cycle_rainbow_chasing == 10){
		// 红色
	...
	} else if(cnt_for_cycle_rainbow_chasing == 3  || cnt_for_cycle_rainbow_chasing == 9){
		// 红色
	...
	} else if(cnt_for_cycle_rainbow_chasing == 4 || cnt_for_cycle_rainbow_chasing == 8){
		// 红色
	...
	} else if(cnt_for_cycle_rainbow_chasing == 5  || cnt_for_cycle_rainbow_chasing == 7){
				// 红色
		led_config_logic[5].red = 255;
		// 绿色
	...
	} else if(cnt_for_cycle_rainbow_chasing == 6){
				// 红色
		led_config_logic[6].red = 255;
		// 绿色
...
static void color_chasing_7_half_cycle_reached() {
	...
	if(!cnt_for_cycle_color_chasing_7){
		...
		// 粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝/蓝
		...
		// 蓝/粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝
		...
		// 浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿
		...
		// 翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白/红/绿
		...
		// 绿/翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白/红
		led_config_logic[0].green = 255;
		...
		// 红/绿/翠绿/浅蓝/蓝/粉红/白/红/绿/翠绿/浅蓝/蓝/粉红/白
		led_config_logic[0].red = 255;
