...
static void PWMx_GPIO_Set(const u8 PWM_id, const bool gpio_output) {
    if (PWM_id == PWM3_ID) {
        PWM3 = gpio_output;
    } else if (PWM_id == PWM4_ID) {
        PWM4 = gpio_output;
    } else if (PWM_id == PWM5_ID) {
        PWM5 = gpio_output;
    } else if (PWM_id == PWM6_ID) {
        PWM6 = gpio_output;
...
void PWM_Init()
{
    ...
    PWMx_InitStructure.PWMx_IO_Select       = PWM3_P21;     
    ...
    PWMx_Configuration(PWM3_ID, &PWMx_InitStructure);                                       

    PWMx_InitStructure.PWMx_IO_Select   = PWM4_P22;     
    PWMx_Configuration(PWM4_ID, &PWMx_InitStructure);                                           

    PWMx_InitStructure.PWMx_IO_Select   = PWM5_P23;     
    PWMx_Configuration(PWM5_ID, &PWMx_InitStructure);                                           

    PWMx_InitStructure.PWMx_IO_Select   = PWM6_P16;     
    PWMx_Configuration(PWM6_ID, &PWMx_InitStructure);                                           
    ...
    // IO config
    push_pull(PWM_R_Mx, PWM_R_BIT);
    push_pull(PWM_G_Mx, PWM_G_BIT);
    push_pull(PWM_B_Mx, PWM_B_BIT);
...
void PWM_Pin_Init() {
    PWMx_Setup_DutyCycle(PWM_R, 0);
    PWMx_Setup_DutyCycle(PWM_G, 0);
    PWMx_Setup_DutyCycle(PWM_B, 0);
